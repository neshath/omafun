...
test('runtime mute and disposal affect active audio and clean failed playback',async()=>{
  const OriginalAudio=globalThis.Audio;
  const OriginalContext=globalThis.AudioContext;
  const instances=[];
  class FakeAudio{
    constructor(src){this.src=src;this.muted=false;this.loop=false;this.currentTime=0;this.volume=1;this.paused=false;instances.push(this);}
    async play(){if(FakeAudio.reject)throw Error('play rejected');this.paused=false;}
