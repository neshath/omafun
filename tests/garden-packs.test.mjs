...
import {templateProject,packExampleProject} from '../src/templates.js';
...
for(const [id,pack] of Object.entries(gardenPacks))test(id+' pack renders and round trips',()=>{
 assert.equal(biomeNames[id],pack.name);assert.equal(palettes[id].length,16);assert.equal(pack.tiles.length,4);assert.ok(pack.landmarks.length>=2);assert.ok(pack.landmarks.every(kind=>gardenProps[kind]));
